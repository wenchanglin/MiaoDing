    ...
    [getData getData:URL_GetYouPingList PostParams:params finish:^(BaseDomain *domain, Boolean success) {
        if ([self checkHttpResponseResultStatus:domain]) {
            ...
            for (NSMutableDictionary *dic in detailArray) {
                chageDiyModel *model = [chageDiyModel new];
                model.imageAvater = [dic stringForKey:@"thumb"];
                model.name = [dic stringForKey:@"name"];
                model.sub_name = [dic stringForKey:@"sub_name"];
...
-(void)viewCreate
{
    ...
    clothesCollection = [[UICollectionView alloc]initWithFrame:CGRectMake(8, 64, SCREEN_WIDTH - 16, SCREEN_HEIGHT -64- 49) collectionViewLayout:flowLayout];
    
    //设置代理
    clothesCollection.delegate = self;
    clothesCollection.dataSource = self;
    clothesCollection.showsVerticalScrollIndicator = FALSE;
    ...
    clothesCollection.backgroundColor = getUIColor(Color_clothDiyBack);
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell sizeToFit];
    [cell setBackgroundColor:getUIColor(Color_clothDiyBack)];
    if (!cell) {
...
//    [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", PIC_HEADURL, [detailArray[indexPath.item] stringForKey:@"thumb"]]]];
//    [cell.tagLabel setText:[detailArray[indexPath.item] stringForKey:@"sub_name"]];
//    [cell.titleLabel setText:[detailArray[indexPath.item] stringForKey:@"name"]];
    cell.model = model;
    return cell;
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return CGSizeMake(SCREEN_WIDTH / 2 - 8, (SCREEN_HEIGHT - 64 - 49) / 2);
}
