...
- (void)setViews
{
    ...
    self.pageV.pageIndicatorTintColor = [UIColor colorWithHexString:@"#ABABAB"];// 设置非选中页的圆点颜色
    self.pageV.currentPageIndicatorTintColor = [UIColor colorWithHexString:@"#222222"]; // 设置选中页的圆点颜色
    self.btn.frame = CGRectMake(0, 0, 166, 37);
    self.btn.center = CGPointMake(self.frame.size.width/2,[ShiPeiIphoneXSRMax isIPhoneX]?self.pageV.frame.origin.y - self.btn.frame.size.height/2+10:self.pageV.frame.origin.y - self.btn.frame.size.height/2);
    self.btn.hidden = YES;
...
//    [self.btn setTitleColor:[UIColor colorWithHexString:@"#E0BE8D"] forState:UIControlStateNormal];
    [self.btn setBackgroundImage:[UIImage imageNamed:@"开始体验"] forState:UIControlStateNormal];
