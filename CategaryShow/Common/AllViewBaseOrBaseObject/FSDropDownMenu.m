...
//  FSDropDownMenu
//
//  Created by hmw on 15/7/14.
//  Copyright (c) 2015年 hmw. All rights reserved.
//
...
@end
#define kButtomImageViewHeight 15
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x+ScreenWidth*0.7, self.frame.origin.y + self.frame.size.height, ScreenWidth*0.7, 0) style:UITableViewStylePlain];
...
- (void)animateBackGroundView:(UIView *)view show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        _buttomImageView.frame = CGRectMake(self.origin.x,200, self.frame.size.width, kButtomImageViewHeight);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            _buttomImageView.frame = CGRectMake(0, CGRectGetMaxY(_leftTableView.frame)-2, ScreenWidth, kButtomImageViewHeight);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView == _rightTableView){
        ...
        sView.backgroundColor = [UIColor colorWithWhite:0.9 alpha:1.f];
