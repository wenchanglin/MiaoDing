...
-(void)createCollection
{
    ...
    [_confirm.layer setBorderColor:[UIColor lightGrayColor].CGColor];
    [_confirm.layer setBorderWidth:1];
    [_confirm.titleLabel setFont:[UIFont systemFontOfSize:11.5]];
    [_confirm setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];

    
    _logistics = [UIButton new];
    [self addSubview:_logistics];
    _logistics.sd_layout
    .rightSpaceToView(_confirm, 13)
    .topSpaceToView(line2, 9.5)
    .widthIs(70)
    .heightIs(25);
    [_logistics.layer setCornerRadius:1];
    ...
    [_logistics.titleLabel setFont:[UIFont systemFontOfSize:11.5]];
...
{
    if ([self.delegate respondsToSelector:@selector(orderCompelClickItemToDetailWithOrderSn:withStatus:)]) {
        [self.delegate orderCompelClickItemToDetailWithOrderSn:_model.order_sn withStatus:_model.status];
    }
}
