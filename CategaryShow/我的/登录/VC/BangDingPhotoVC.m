...
@interface BangDingPhotoVC ()<YBAttributeTapActionDelegate,UITextFieldDelegate>
...
@implementation BangDingPhotoVC
{
...
-(void)setUp{
   ...
   // [buttonClose setImage:[UIImage imageNamed:@"X"] forState:UIControlStateNormal];
    ...
    login = [UIButton new];
    [login setTitle:@"绑定手机" forState:(UIControlStateNormal)];
...
- (void)LoginClick{
    ...
    [[LoginManager getInstance] postLoginAuth:username.text userPwd:checkCount.text loginId:_userid userId:_userid icon:_icon nickName:_nickname isType:_is_type isAuto:YES finish:^(Boolean success) {
        if (success) {
