...
-(void)getDatas
{
    ...
    [getData getData:[MoreUrlInterface URL_OrderDetailForOrdersn_String] PostParams:params finish:^(BaseDomain *domain, Boolean success) {
...
//                model.spec_content = [dic stringForKey:@"spec_content"];
//                model.diy_content = [dic stringForKey:@"diy_content"];
//                if ([model.good_type integerValue]==1||[model.good_type integerValue]==3) {//定制
//                    model.sizeOrDing = [NSString stringWithFormat:@"%@;%@",model.spec_content,model.diy_content];
//                } else {//成品
...
-(void)createTableView
{
    ...
    if (_canCommend) {
        commendButton = [[UIButton alloc] initWithFrame:CGRectMake(0,[ShiPeiIphoneXSRMax isIPhoneX]?SCREEN_HEIGHT-74-84:SCREEN_HEIGHT -64- 49, SCREEN_WIDTH, 49)];
